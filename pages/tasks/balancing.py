...
def visualize_steps_with_overlay():
    ...
    if "steps" in st.session_state and "ship_grids" in st.session_state:
        ...
        step_number = st.number_input("Select Container (Step)", min_value=1, max_value=total_steps, value=1, step=1) - 1
...
def balancing_page():
    ...
    if st.button("Calculate Initial Balance"):
        left_balance, right_balance, _ = calculate_balance(st.session_state.ship_grid)
        ...
        if abs(left_balance - right_balance) == 0:
            ...
            st.warning("The ship is slightly unbalanced but close to being balanced.")
        else:
            st.error("The ship is significantly unbalanced. Balancing is highly recommended.")
    # Perform balancing
