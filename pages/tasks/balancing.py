...
def balancing_page():
    ...
    # Use manuscript from file_handler to initialize the grid
    ...
    st.subheader("Current Ship Layout")
    st.plotly_chart(
    ...
    # Perform balancing
    if st.button("Balance Ship", key="balance_ship"):
        try:
            ...
            if balanced:
                ...
                steps, ship_grids, status = balance(
                    st.session_state["ship_grid"], st.session_state.get(
                        "containers", [])
                )
                st.session_state["steps"] = steps
                # Store intermediate grids
                st.session_state["ship_grids"] = ship_grids
                # Update the final grid
                st.session_state["ship_grid"] = ship_grids[-1]

                # Save updated grid and visualization
                st.session_state["updated_grid"] = ship_grids[-1]
                st.session_state["final_plot"] = plotly_visualize_grid(
                    st.session_state["updated_grid"], title="Balanced Ship Layout"
                )
    ...
    # Display the final grid after balancing, if available
    if st.session_state.get("final_plot"):
        st.subheader("Balanced Ship Layout")
        ...
        # Provide download button for updated manuscript
        st.download_button(
